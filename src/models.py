...
class VisionModelForCLS(torch.nn.Module):
    def __init__(self,
        ...
        self.batch_per_class_ap_metric = torchmetrics.AveragePrecision(
            task="multilabel", num_labels=self.num_labels, average=None 
        ).to(self.device)
        
        # F1 Score metrics
        self.batch_overall_f1_metric = torchmetrics.F1Score(
            task="multilabel", num_labels=self.num_labels, average='macro', threshold=0.5
        ).to(self.device)
        self.batch_per_class_f1_metric = torchmetrics.F1Score(
            task="multilabel", num_labels=self.num_labels, average=None, threshold=0.5
        ).to(self.device)
    ...
    def calculate_std_metrics(self, logits: torch.Tensor, labels: torch.Tensor) -> Dict[str, Any]:
        labels_int = labels.to(logits.device).int() 
        prob_scores = torch.sigmoid(logits)
        # For F1Score, it's better to pass probabilities if the metric handles thresholding,
        # or pass binarized predictions if you manage thresholding (e.g., 0.5)
        preds_int = (prob_scores > 0.5).int() # For F1 and other discrete metrics
        preds_float = preds_int.float() # For acc, em that compare with float labels

        # --- Accuracy and Exact Match ---
        overall_acc = (preds_float == labels).float().mean().item() # labels are float here
        ...
        per_class_aps = self.batch_per_class_ap_metric.compute()
        ...
        self.batch_overall_f1_metric.update(preds_int, labels_int) # F1 uses binarized predictions
        f1_overall_val = self.batch_overall_f1_metric.compute()
        self.batch_overall_f1_metric.reset()

        self.batch_per_class_f1_metric.update(preds_int, labels_int)
        per_class_f1s = self.batch_per_class_f1_metric.compute()
        self.batch_per_class_f1_metric.reset()
        
        f1_privileged_val = torch.tensor(np.nan, device=logits.device)
        if priv_indices_list and per_class_f1s is not None and per_class_f1s.numel() == self.num_labels:
            f1s_for_privileged = per_class_f1s[priv_indices_list]
            valid_priv_f1s = f1s_for_privileged[~torch.isnan(f1s_for_privileged)]
            if valid_priv_f1s.numel() > 0:
                f1_privileged_val = valid_priv_f1s.mean()

        f1_non_privileged_val = torch.tensor(np.nan, device=logits.device)
        if non_priv_indices_list and per_class_f1s is not None and per_class_f1s.numel() == self.num_labels:
            f1s_for_non_privileged = per_class_f1s[non_priv_indices_list]
            valid_non_priv_f1s = f1s_for_non_privileged[~torch.isnan(f1s_for_non_privileged)]
            if valid_non_priv_f1s.numel() > 0:
                f1_non_privileged_val = valid_non_priv_f1s.mean()
        ...
        def to_loggable(val_tensor):
            ...
            elif isinstance(val_tensor, float) and np.isnan(val_tensor):
                return np.nan
            return val_tensor
...
def main_cls(model_name: str, device: torch.device) -> None:
    ...
    def print_output_dict(name, output_dict):
        ...
        # Updated print for metrics
        for metric_name, metric_dict in output_dict.items():
            if metric_name in ["acc", "em", "f1", "map"]:
                print(f"  {metric_name.upper()} Components:")
                if isinstance(metric_dict, dict):
                    for k_group, v_metric in metric_dict.items():
                        # Adjust key for 'map' to match its internal structure 'mAP'
                        display_key = k_group if metric_name != 'map' else ('mAP' if k_group == 'mAP' else k_group)
                        print(f"    {display_key}: {v_metric}")
                else: # Should not happen if to_loggable works correctly
                    print(f"    {metric_name}: {metric_dict}")
