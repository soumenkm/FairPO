...
if __name__ == "__main__":
    os.environ["CUDA_VISIBLE_DEVICES"] = "1" # Example, adjust as needed
...
class ModelTrainer:
    def __init__(self, config):
        ...
        self.test_step = 0 # Initialize test step counter
        ...
        self.test_dataset = COCODatasetOnDemand(
             root_dir=config.coco_root, frac=config.val_frac, is_train=False, # Usually test uses val split or a dedicated test split
             privileged_indices_set=privileged_indices_set, seed=seed,
             index_dir=config.index_dir, force_regenerate=config.force_regenerate_index
        )
        ...
        # Watch model *after* it's initialized and potentially moved to device
        if self.config.wandb_project and wandb.run:
             try:
                 wandb.watch(self.model.model, log="all", log_freq=config.log_freq*5) # Watch the inner transformer model
        ...
        if config.resume_checkpoint:
            ...
            # self.global_step is loaded directly from checkpoint now
    ...
    def _setup_wandb(self):
        if self.config.wandb_project:
            try:
                ...
                # self.checkpoint_dir = Path(self.config.checkpoint_dir) # Already done in init
                # self.checkpoint_dir.mkdir(parents=True, exist_ok=True)
                wandb.init(
                    ...
                    id=self.config.wandb_id, # For resuming runs
                ...
                logging.info(f"WandB initialized for project '{self.config.wandb_project}', run '{self.config.run_name}'.")
                 ...
                 wandb.run = None # Ensure wandb.run is None
        else:
    ...
    def _save_checkpoint(self, epoch, is_best=False):
        ...
        checkpoint_data = {
            ...
            'val_step': self.val_step, # Save validation step counter too
        }
    ...
    def _load_checkpoint(self, checkpoint_path):
        ...
        if checkpoint_path.is_file():
            ...
            self.val_step = checkpoint.get('val_step', self.start_epoch) # Resume validation step
            ...
            logging.info(f"Resumed training from epoch {self.start_epoch}, global step {self.global_step}, val step {self.val_step}")
        else:
    ...
    def train(self):
        ...
        for epoch in range(self.start_epoch, self.config.epochs):
            ...
            for i, batch in enumerate(batch_iter):
                ...
                if self.config.is_ref_training:
                    ...
                    if not torch.isnan(loss):
                        ...
                        logging.warning(f"NaN SFT loss detected at step {self.global_step}. Skipping backward.")
                    ...
                    # Check for NaN before using losses
                ...
                # Loss should be valid here due to checks above
                loss.backward()
                ...
                # --- Optimizer Step ---
                self.optimizer.step()
                self.global_step += 1 # Increment global step *after* successful step

                # --- Calculate Parameter Norm ---
                # Calculate after step if you want the norm of updated params
                ...
                batch_log_payload["train/step"] = self.global_step # Add the step itself to log
            ...
            avg_metrics = {k: v / processed_items for k, v in epoch_metrics.items() if k.startswith('loss') or k.startswith('acc')}
            # For norms, simple average over batches where it was computed
            num_batches_processed = len(batch_iter)
            avg_metrics['grad_norm'] = epoch_metrics['grad_norm'] / num_batches_processed if num_batches_processed > 0 else 0
            avg_metrics['param_norm'] = epoch_metrics['param_norm'] / num_batches_processed if num_batches_processed > 0 else 0
            ...
            if wandb.run:
                # Log Train Epoch Summaries (against train/step)
                train_epoch_log = {
                    "epoch": epoch + 1,
                    ...
                    "train/step": self.global_step # Include the step
                }
                if self.config.is_ref_training:
                    train_epoch_log["train/avg_sft_loss"] = avg_metrics.get('loss_sft', np.nan)
                else:
                    train_epoch_log.update({
                ...
                wandb.log(train_epoch_log) # Log train epoch metrics (uses train/step)

                # Log Validation Metrics (against val/step)
                if val_metrics: # Only log if validation produced metrics
            ...
            if is_best:
                ...
                logging.info(f"New best validation metric: {self.best_val_metric:.4f} at epoch {epoch+1} (global step {self.global_step})")
                if wandb.run:
                     wandb.summary['best_val_metric'] = self.best_val_metric
                     wandb.summary['best_epoch'] = epoch + 1
                     wandb.summary['best_global_step'] = self.global_step
            self._save_checkpoint(epoch, is_best=is_best)

        logging.info("Training finished.")
        # WandB finish is handled in main()


    def _validate(self, epoch):
        logging.info(f"Starting validation for epoch {epoch+1} (global step {self.global_step})...")
        self.model.eval()
        val_metrics = {
            'loss_priv': 0.0, 'loss_non_priv': 0.0, 'loss_total': 0.0, 'loss_sft': 0.0,
            'acc_priv': 0.0, 'acc_non_priv': 0.0, 'acc_overall': 0.0
        ...
        with torch.no_grad():
            ...
            for batch in val_iter:
                ...
                if self.config.is_ref_training:
                    ...
                    valid_loss_priv = not torch.isnan(loss_priv)
                    valid_loss_non_priv = not torch.isnan(loss_non_priv)

                    if valid_loss_priv: val_metrics['loss_priv'] += loss_priv.item() * current_batch_size
                    if valid_loss_non_priv: val_metrics['loss_non_priv'] += loss_non_priv.item() * current_batch_size

                    # Use current training alphas for combined loss calculation (consistent reporting)
                    if valid_loss_priv and valid_loss_non_priv:
                        # Ensure alphas exist (might not if loading old checkpoint)
    ...
    def test(self, checkpoint_path=None):
        ...
        if load_path is None:
            load_path = self.checkpoint_dir / "checkpoint_best.pth"
        else:
            load_path = Path(load_path) # Ensure it's a Path object

        # Ensure reference weights are loaded for FairPO testing if needed
        ref_weights_loaded = False
        if not self.config.is_ref_training:
             if not hasattr(self.model, 'ref_classifier') or self.model.ref_classifier is None:
             ...
             if ref_weights_file and ref_weights_file.is_file():
                  try:
                      ref_state_dict = torch.load(ref_weights_file, map_location=self.device)
                      # Handle cases where weights might be saved directly or within a dict
                      if 'model_state_dict' in ref_state_dict:
                           ref_state_dict = ref_state_dict['model_state_dict']
                      elif 'state_dict' in ref_state_dict:
                           ref_state_dict = ref_state_dict['state_dict']

                      self.model.ref_classifier.load_state_dict(ref_state_dict)
                      ...
                      logging.warning("Attempting to load reference weights with strict=False")
                      try:
                          self.model.ref_classifier.load_state_dict(ref_state_dict, strict=False)
                          ref_weights_loaded = True
                      except Exception as e_strict_false:
                           logging.error(f"Failed to load reference weights even with strict=False: {e_strict_false}")
        ...
        if load_path.is_file():
             logging.info(f"Loading model checkpoint for testing: {load_path}")
             checkpoint = torch.load(load_path, map_location=self.device)
             classifier_state_dict = None
             if 'model_state_dict' in checkpoint:
                 classifier_state_dict = checkpoint['model_state_dict']
             elif isinstance(checkpoint, dict) and any(k.startswith('model.classifier') for k in checkpoint.keys()): # Check if it's a raw state dict
                 classifier_state_dict = checkpoint
             elif isinstance(checkpoint, dict) and 'state_dict' in checkpoint: # Another common pattern
                  classifier_state_dict = checkpoint['state_dict'] # Adjust if classifier is nested differently
             else: # Assume the file IS the state dict
                  classifier_state_dict = checkpoint

             if classifier_state_dict:
                 try:
                    self.model.model.classifier.load_state_dict(classifier_state_dict)
                    ...
                    logging.warning("Attempting to load with strict=False")
                    try:
                        self.model.model.classifier.load_state_dict(classifier_state_dict, strict=False)
                        logging.info("Loaded classifier weights with strict=False.")
                    except Exception as e_strict:
                        logging.error(f"Failed loading test checkpoint even with strict=False: {e_strict}")
                        return {} # Cannot proceed
             else:
                 logging.error(f"Could not extract classifier state_dict from checkpoint file: {load_path}")
                 return {}
        else:
            logging.error(f"Test checkpoint not found at {load_path}. Cannot perform test.")
            return {}
        ...
        processed_items = 0
        self.test_step = 0 # Reset or initialize test step for this run
        ...
        with torch.no_grad():
            ...
            for batch in test_iter:
                ...
                loss_components = output_dict['loss']
                acc_components = output_dict['acc']
                batch_test_metrics = {} # For potential per-batch test logging

                 # Accumulate metrics (similar to validation, weighted sum)
                if self.config.is_ref_training:
                    loss = loss_components.get("loss", torch.tensor(np.nan, device=self.device))
                    if not torch.isnan(loss):
                        test_metrics['loss_sft'] += loss.item() * current_batch_size
                        batch_test_metrics['test/sft_loss'] = loss.item()
                else:
                    loss_priv = loss_components.get("privileged", torch.tensor(np.nan, device=self.device))
                    loss_non_priv = loss_components.get("non_privileged", torch.tensor(np.nan, device=self.device))
                    valid_loss_priv = not torch.isnan(loss_priv)
                    ...
                    if valid_loss_priv:
                        ...
                        batch_test_metrics['test/privileged_loss'] = loss_priv.item()
                    if valid_loss_non_priv:
                        test_metrics['loss_non_priv'] += loss_non_priv.item() * current_batch_size
                        batch_test_metrics['test/non_privileged_loss'] = loss_non_priv.item()
                    ...
                    if valid_loss_priv and valid_loss_non_priv:
                        ...
                        batch_test_metrics['test/combined_loss'] = combined_loss.item()
                ...
                if not np.isnan(acc_priv_val):
                     test_metrics['acc_priv'] += acc_priv_val * current_batch_size
                     batch_test_metrics['test/accuracy_privileged'] = acc_priv_val
                if not np.isnan(acc_non_priv_val):
                     test_metrics['acc_non_priv'] += acc_non_priv_val * current_batch_size
                     batch_test_metrics['test/accuracy_non_privileged'] = acc_non_priv_val
                if not np.isnan(acc_overall_val):
                     test_metrics['acc_overall'] += acc_overall_val * current_batch_size
                     batch_test_metrics['test/accuracy_overall'] = acc_overall_val


                processed_items += current_batch_size
                self.test_step += 1 # Increment per batch

                # Log per-batch test metrics if desired (optional)
                # if wandb.run and batch_test_metrics:
                #    batch_test_metrics['test/step'] = self.test_step
                #    wandb.log(batch_test_metrics)
        ...
        if self.config.is_ref_training:
             ...
             logging.info(f"  Avg Loss (Combined): {avg_test_metrics.get('test/loss_total', np.nan):.4f}") # Uses 'loss_total'
        ...
        # Log final average test results to WandB
        if wandb.run:
            # Log summary metrics against a single test step (e.g., the final one)
            avg_test_metrics['test/step'] = self.test_step # Log total number of test batches processed
            wandb.log(avg_test_metrics) # Log test metrics (uses test/step)

            # Also update WandB summary table for easy access
            for key, value in avg_test_metrics.items():
                 if key != 'test/step': # Don't pollute summary with step
                    summary_key = key.replace('/', '_') # e.g., test_acc_overall
                    wandb.summary[summary_key] = value
...
def main():
    ...
    parser.add_argument('--run_name', type=str, default="FairPO_fixed", help='Custom WandB run name (e.g., FairPO_b64_lr1e-5_eta0.01)')
    ...
    if args.wandb_project and not args.run_name:
        ...
        logging.info(f"Generated WandB run name: {args.run_name}")
         ...
         args.run_name = "local_run" # Assign a default if WandB is off

    # Argument Validation
    # Check COCO root exists
    if not Path(args.coco_root).is_dir():
         logging.error(f"--- ERROR --- COCO root directory not found at: {args.coco_root}")
         logging.error("Please set the --coco_root argument correctly.")
         sys.exit(1)

    # Check reference weights path
    if not args.is_ref_training and not args.test_only: # FairPO Training
        if not args.ref_cls_weights_path:
            logging.error("--- ERROR --- --ref_cls_weights_path is REQUIRED for FairPO training (when --is_ref_training is not set).")
         ...
         if not args.ref_cls_weights_path or not Path(args.ref_cls_weights_path).is_file():
              ...
              logging.warning("Test results may be inaccurate as the reference model might use initial/incorrect weights.")
              logging.warning("-----------------")
    ...
    try:
        ...
        logging.error(f"An error occurred during execution: {e}", exc_info=True) # Log traceback
        # Optionally: Log error to WandB if run is active
        if wandb.run:
