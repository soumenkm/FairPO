...
class ModelTrainer:
    def __init__(self, config):
        ...
        self.model = VisionModelForCLS(
            ...
            is_ref=config.is_ref_training, loss_type=config.loss_type, beta=config.beta, epsilon=config.epsilon,
            quant_config=None 
        ).to(self.device)
...
def main():
    ...
    # Paths
    ref_cls_weights_path = None # f"{current_dir}/output/ckpt/ref_model/ckpt_ep_latest.pth" # !IMP: CHANGE THIS
    ...
    parser.add_argument('--loss_type', default=None, help='Loss type (dpo, simpo, cpo, None)')
